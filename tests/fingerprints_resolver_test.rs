...
//
// Duplicated from the other integration tests because each
// `tests/*.rs` file compiles as its own binary and cannot share
// helper modules without a common module subdir.
...
async fn spawn_schema_service() -> SpawnedService {
    ...
    let db_path = temp_dir
        .path()
        .join("resolver_test_schema_registry")
        .to_string_lossy()
        .to_string();
