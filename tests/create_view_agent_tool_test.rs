...
mod tests {
    ...
    async fn agent_tool_create_view_word_count() {
        ...
        let tool_params = json!({
            ...
            "rust_transform": "fn transform_impl(input: Value) -> Value {\n    let empty = vec![];\n    let inputs = &input[\"inputs\"];\n    let posts = inputs[\"BlogPost\"].as_array().unwrap_or(&empty);\n    \n    // Output format: { \"fields\": { \"word_count\": { key: val, ... }, \"content_preview\": { key: val, ... } } }\n    let mut word_count_map = serde_json::Map::new();\n    let mut default_key = 0;\n    let mut content_preview_map = serde_json::Map::new();\n    \n    // Handle generic inputs which is an object of schemas mapping to an object of fields mapping to keys\n    // In the old tests it was an array, wait actually the inputs are in the new format too!\n    let fields_obj = inputs[\"BlogPost\"][\"content\"].as_object();\n    if let Some(fields) = fields_obj {\n        for (k, v) in fields {\n            let content = v.as_str().unwrap_or(\"\");\n            let word_count = content.split_whitespace().count();\n            let preview: String = content.chars().take(50).collect();\n            word_count_map.insert(k.clone(), serde_json::json!(word_count));\n            content_preview_map.insert(k.clone(), serde_json::json!(preview));\n        }\n    }\n    serde_json::json!({\n        \"fields\": {\n            \"word_count\": word_count_map,\n            \"content_preview\": content_preview_map\n        }\n    })\n}"
        });
    ...
    async fn agent_tool_multi_schema_join_view() {
        ...
        let tool_params = json!({
            ...
            "rust_transform": "fn transform_impl(input: Value) -> Value {\n    let empty_obj = serde_json::Map::new();\n    let inputs = &input[\"inputs\"];\n    let blog_post = inputs[\"BlogPost\"].as_object().unwrap_or(&empty_obj);\n    let titles = blog_post.get(\"title\").and_then(|t| t.as_object()).unwrap_or(&empty_obj);\n    let authors = blog_post.get(\"author\").and_then(|a| a.as_object()).unwrap_or(&empty_obj);\n    let contents = blog_post.get(\"content\").and_then(|c| c.as_object()).unwrap_or(&empty_obj);\n\n    let mut summary_map = serde_json::Map::new();\n    for (k, title_v) in titles {\n        let title = title_v.as_str().unwrap_or(\"?\");\n        let author = authors.get(k).and_then(|v| v.as_str()).unwrap_or(\"?\");\n        let content = contents.get(k).and_then(|v| v.as_str()).unwrap_or(\"\");\n        let words = content.split_whitespace().count();\n        let summary = format!(\"{} by {} ({} words)\", title, author, words);\n        summary_map.insert(k.clone(), serde_json::json!(summary));\n    }\n    serde_json::json!({\n        \"fields\": { \"summary\": summary_map }\n    })\n}"
        });
    ...
    async fn agent_tool_created_view_shows_in_list() {
        ...
        let tool_params = json!({
            ...
            "rust_transform": "fn transform_impl(input: Value) -> Value {\n    let empty_obj = serde_json::Map::new();\n    let inputs = &input[\"inputs\"];\n    let blog_post = inputs[\"BlogPost\"].as_object().unwrap_or(&empty_obj);\n    let titles = blog_post.get(\"title\").and_then(|t| t.as_object()).unwrap_or(&empty_obj);\n\n    let mut title_upper_map = serde_json::Map::new();\n    for (k, title_v) in titles {\n        let title = title_v.as_str().unwrap_or(\"\").to_uppercase();\n        title_upper_map.insert(k.clone(), serde_json::json!(title));\n    }\n    serde_json::json!({\n        \"fields\": { \"title_upper\": title_upper_map }\n    })\n}"
        });
