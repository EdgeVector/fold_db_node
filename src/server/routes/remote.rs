...
async fn resolve_sender_info() -> Result<SenderInfo, crate::handlers::HandlerError> {
    use crate::discovery::connection;

    let (discovery_url, master_key, auth_token) =
        crate::server::routes::discovery::resolve_discovery_config(None).await?;
...
pub async fn async_query(
    ...
    handler_result_to_response(
        async {
            ...
            let sender = resolve_sender_info().await?;
