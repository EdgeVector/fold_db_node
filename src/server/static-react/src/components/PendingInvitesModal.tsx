...
type SetPendingInvitesAction = OrgInviteBundle[] | ((prev: OrgInviteBundle[]) => OrgInviteBundle[]);

interface PendingInvitesModalProps {
  isOpen: boolean;
  onClose: () => void;
  pendingInvites: OrgInviteBundle[];
  setPendingInvites: (action: SetPendingInvitesAction) => void;
}

export default function PendingInvitesModal({ isOpen, onClose, pendingInvites, setPendingInvites }: PendingInvitesModalProps) {
  ...
  const inviteId = (invite: OrgInviteBundle): string => invite.org_hash || invite.org_public_key || invite.org_name;

  const handleAccept = async (invite: OrgInviteBundle) => {
    ...
    try {
      ...
      if (res.data !== undefined) {
        setPendingInvites((prev: OrgInviteBundle[]) => {
      ...
      setError(err instanceof Error ? err.message : 'Failed to join organization. Try again.');
    } finally {
      setLoadingIds(prev => {
  ...
  const handleDecline = async (invite: OrgInviteBundle) => {
    const id = inviteId(invite);
    try {
      setLoadingIds(prev => new Set(prev).add(id));
      setError(null);
