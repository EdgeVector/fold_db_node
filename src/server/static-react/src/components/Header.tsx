...
import { orgClient, type OrgInviteBundle } from '../api/clients/orgClient'
...
interface HeaderProps {
  ...
  onCloudSettingsClick: () => void
}

type PendingInvite = OrgInviteBundle

function classifySchemaEnv(url: string | null | undefined): SchemaEnvBadge {
...
function Header({ onSettingsClick, onAiSettingsClick, onCloudSettingsClick }: HeaderProps) {
  ...
  const [_schemaEnv, setSchemaEnv] = useState<SchemaEnvBadge | null>(null)
  ...
  useEffect(() => {
    systemClient.getDatabaseConfig().then(res => {
      if (res.data) {
        ...
        const sizeBytes = (res.data as { storage_size_bytes?: number }).storage_size_bytes
        if (sizeBytes) setStorageSize(sizeBytes)
      }
    }).catch(() => { /* best-effort - header info is non-critical */ })
    systemClient.getSystemStatus().then(res => {
      if (res.data) setSchemaEnv(classifySchemaEnv((res.data as { schema_service_url?: string }).schema_service_url))
    }).catch(() => { /* best-effort - header info is non-critical */ })
  ...
  useEffect(() => {
    ...
    const fetchInvites = async () => {
      ...
      try {
        ...
        const data = res.data as { invites?: PendingInvite[] } | PendingInvite[] | undefined
        const invites: PendingInvite[] = Array.isArray(data)
          ? data
          : (data?.invites ?? []);
    ...
    const checkOrgSync = async () => {
      try {
        ...
        const hasError = statuses.some(s => {
          const d = (s && typeof s === 'object' && 'data' in s ? (s as { data?: unknown }).data : s) as { last_error?: unknown } | null
          return Boolean(d?.last_error)
        })
  ...
  return (
    <header className="bg-surface border-b border-border px-4 sm:px-8 py-1.5 flex-shrink-0">
      <div className="flex items-center justify-between min-w-0">
        <div className="flex items-center gap-3 sm:gap-6 min-w-0">
          <a href="/" className="flex items-center gap-2 text-base font-medium tracking-tight text-primary no-underline hover:text-primary flex-shrink-0">
      ...
      <PendingInvitesModal
        ...
        setPendingInvites={(updater: PendingInvite[] | ((prev: PendingInvite[]) => PendingInvite[])) => {
