...
import type { DatabaseConfigDto } from '../../api/clients/systemClient'
...
type DbType = DatabaseConfigDto['type']
...
interface DatabaseConfigHook extends SettingsPanelHook {
  saveDatabaseConfig: () => Promise<void>
}

function useDatabaseConfig({ configSaveStatus, setConfigSaveStatus, onClose }: SettingsPanelProps): DatabaseConfigHook {
  const [dbType, setDbType] = useState<DbType>('local')
  const [dbPath, setDbPath] = useState<string>('data')
  const [isResetting, setIsResetting] = useState(false)
  const [resetResult, setResetResult] = useState<ResetResult | null>(null)
  const pollIntervalRef = useRef<IntervalHandle | null>(null)
  const fallbackTimeoutRef = useRef<TimeoutHandle | null>(null)
  const reloadTimeoutRef = useRef<TimeoutHandle | null>(null)
  ...
  const saveDatabaseConfig = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        setConfigSaveStatus({ success: true, message: data.requires_restart ? 'Saved. Please restart server.' : data.message || 'Saved and restarted' })
        statusTimeoutRef.current = setTimeout(() => { setConfigSaveStatus(null); if (!data.requires_restart) onClose() }, 3000)
  ...
  const handleResetDatabase = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        // `job_id` is read off the response at runtime but is not declared in
        // the OpenAPI-generated `ResetDatabaseResponse` schema. Either the
        // backend should add it to the schema, or this branch is dead code —
        // tracked separately, preserving prior runtime behavior here.
        const data = response.data as typeof response.data & { job_id?: string }
        if (data.job_id) {
          const jobId = data.job_id
          pollIntervalRef.current = setInterval(async () => {
            try {
              const pr = await ingestionClient.getJobProgress(jobId)
              if (pr.success && pr.data) {
                if (pr.data.is_complete) { if (pollIntervalRef.current) clearInterval(pollIntervalRef.current); pollIntervalRef.current = null; setResetResult({ type: 'success', message: 'Reset complete. Reloading...' }); reloadTimeoutRef.current = setTimeout(() => window.location.reload(), 1000) }
                else if (pr.data.is_failed) { if (pollIntervalRef.current) clearInterval(pollIntervalRef.current); pollIntervalRef.current = null; setResetResult({ type: 'error', message: pr.data.error_message || 'Reset failed' }); setIsResetting(false) }
              }
            } catch { /* Polling error - will retry on next interval */ }
          }, 1000)
          fallbackTimeoutRef.current = setTimeout(() => { if (pollIntervalRef.current) clearInterval(pollIntervalRef.current); pollIntervalRef.current = null; fallbackTimeoutRef.current = null; if (isResetting) { setResetResult({ type: 'success', message: 'Reset likely complete. Reloading...' }); reloadTimeoutRef.current = setTimeout(() => window.location.reload(), 1000) } }, 60000)
        } else { setResetResult({ type: 'success', message: data.message || 'Reset successfully' }); reloadTimeoutRef.current = setTimeout(() => window.location.reload(), 2000) }
