...
function AnimatedLogo({ size = 84, className = '' }: AnimatedLogoProps) {
  ...
  useEffect(() => {
    ...
    function getShapeState(p: number, i: number): { t: number; rot: number } {
      const fs = FWD_START[i], fe = fs + SPIN_DUR
      const rs = REV_START[i], re = rs + SPIN_DUR
      let t: number, rot: number
      if (p < fs)      { t = 0; rot = 0 }
    ...
    function drawShape(cx: number, cy: number, hw: number, hh: number, r: number, color: string, sy: number, rot: number, t: number) {
