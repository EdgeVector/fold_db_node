...
function StatusSection() {
  ...
  const handleResetDatabase = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        const data = response.data as { job_id?: string; message?: string }
        if (data.job_id) {
          setResetResult({
            type: 'success',
            message: `Reset started (Job: ${data.job_id.substring(0, 8)}...). Progress will appear above.`
          })
          setShowConfirmDialog(false)
          setIsResetting(false)
        } else {
          setResetResult({ type: 'success', message: data.message || 'Reset complete' })
          setTimeout(() => {
  ...
  const renderJobCard = (job: ProgressJob) => {
    ...
    const cardClass = isDatabaseReset ? 'card card-error' : 'card card-info'
