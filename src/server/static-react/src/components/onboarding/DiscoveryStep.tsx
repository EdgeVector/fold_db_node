...
function InterestTag({ label, selected, onToggle }: InterestTagProps) {
  return (
    <button
      type="button"
      onClick={onToggle}
...
export default function DiscoveryStep({ onNext, onSkip }: DiscoveryStepProps) {
  ...
  const handleOptIn = async () => {
    ...
    const schemasToOptIn = (approvedSchemas || []).slice(0, 5)
    let succeeded = 0
    ...
    if (succeeded === 0 && schemasToOptIn.length > 0) {
      ...
      setError(`Failed to share schemas with the network: ${msg}`)
      setOptingIn(false)
      return
    }
