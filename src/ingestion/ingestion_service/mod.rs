    ...
    async fn process_decomposed_path(
        ...
        if let Some(ref org_hash) = request.org_hash {
            ...
            let unique_names: std::collections::HashSet<String> =
                schema_cache.local_schema_names().into_iter().collect();
            let mut name_map = HashMap::new();
            for name in unique_names {
                let org_name = self.ensure_org_schema(&name, org_hash, node).await?;
                if org_name != name {
...
/// Filter mutation mappers to only reference fields that exist in the schema's
/// runtime_fields. The AI may include mappers for fields it dropped from the
/// schema definition, causing write failures.
pub(crate) fn filter_mappers_by_schema(
