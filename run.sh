...
#
# Options:
#   --local          (Default, kept for compatibility) Local Sled storage
...
for arg in "$@"; do
    case "$arg" in
        ...
        --demo)
            ...
            ;;
        --region=*)
            # Legacy flag, ignored (no DynamoDB mode)
            ;;
        --home)
...
EOF
    ...
    # Default: local Sled storage. FoldDB always starts local.
    # Cloud sync (Exemem) activates when the user registers with an invite code.
